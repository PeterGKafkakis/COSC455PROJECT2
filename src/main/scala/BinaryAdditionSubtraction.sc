...
{
  (remainingBits.isEmpty, carryBit)
  match
    {
...
def getNextCarryBit(pBit: Boolean, qBit: Boolean, carryBit: Boolean): Boolean = {(qBit && carryBit) || (pBit && qBit) ||  (pBit && carryBit)}
...
def doBinaryAddition(pBits: List[Boolean], qBits: List[Boolean], carryBit: Boolean): List[Boolean] = {
  (qBits.isEmpty, pBits.isEmpty, carryBit)
  match
  {
...
def convertIntListToBooleanList(intList: List[Int]) = {
  intList.map { case 0 => false case 1 => true}
}
...
booleanList.map { case false => 0 case true => 1}
}
